...
public class New3 {
    ...
    private final static int unum = 24;
    private final static int vnum = 20;

    private final static double holex = 18;
    private final static double holey = 50;
    private static final double holeSize = 12;

    private static final double TwoPI = Math.PI * 2;

    static boolean secondset = false;
    ...
    public static void main(String[] args) {
        ...
        Point[] listA = { new Point(0, 0, 20), new Point(0, 0, 30),
                new Point(0, 10, 40), new Point(0, 20, 40),
                new Point(0, 40, 20), new Point(-30, 50, 10),
                new Point(-30, 60, 10), new Point(-30, 80, 8),
                new Point(-10, 80, 6), new Point(0, 80, 6), };

        Point[] listB = { new Point(0, 0, 19.9), new Point(0, 0, 15),
                new Point(0, 10, 11), new Point(0, 20, 5),
                new Point(0, 40, 3), new Point(30, 50, 3),
                new Point(30, 60, 3), new Point(30, 80, 5),
                new Point(10, 80, 4 + .5 * thickness),
                new Point(0, 80, 6 + thickness) };
        ...
        int numv = points[0].length - 1;
        // points2[num] = shrunkPoints[num];

        // shrunkPoints2[num] = shrunkPoints[num];

        for (int v = 0; v < vnum; v++) {
            shrunkPoints2[num][v] = points[num][numv - v];
            // shrunkPoints2[num][v].x-=.001;
            points2[num][v] = shrunkPoints[num][numv - v];

            // shrunkPoints2[num][v].x+=10*Math.cos(TwoPI*v/vnum);
            // shrunkPoints2[num][v].y+=10*Math.sin(TwoPI*v/vnum);
        }
        ...
        print2(points, shrunkPoints);

        System.out.println("translate([-.0001,0,0]){");
        print2(points2, shrunkPoints2);
        System.out.println("}\n}");
        // holes(points, points2);
        // System.out.println("}");

        /**
         * print1(points2); print1(shrunkPoints);
         **/
    ...
    private static void print2(Point[][] points, Point[][] points2) {
        round(points);
        round(points2);
        int upoints = points.length;
        int vpoints = points[0].length;
        ...
        for (int i = 0; i < upoints - 1; i++) {
            for (int j = 0; j < vpoints; j++) {
                ...
                // g h
                // e f
                int e = i * vpoints + j + offset;
                int f = ((i + 1) % upoints) * vpoints + j + offset;
                int g = i * vpoints + (j + 1) % vpoints + offset;
                int h = ((i + 1) % upoints) * vpoints + (j + 1) % vpoints
                        + offset;
    ...
    private static Point[][] points(Bezier template) {
        ...
        Point[] centers;
        if (secondset) {
            centers = template.renderSpecial2(upoints + 1, .7);

        } else {
            centers = template.renderSpecial(upoints + 1, .7);
        }
        secondset = !secondset;
    ...
    private static Point[][] shrunkPoints(Bezier template,
        ...
        Point[] tempCenters;
        if (secondset) {
            tempCenters = template.renderSpecial2(upoints + 1, .7);

        } else {
            tempCenters = template.renderSpecial(upoints + 1, .7);
        }
